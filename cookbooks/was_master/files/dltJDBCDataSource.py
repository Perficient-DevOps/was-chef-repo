#*************************************************************************
# Script: dltJDBCProvider.py - Delete JDBC
#
...
#		1) Data source name
#		2) Debug "YES" or "NO" for printing debug statements for script
#
# Return Codes:
...
#		\  -f /WorkingData/jythonScripts/dltJDBCDataSource.py "PlantsByWebSphereDataSource"  "YES"
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
    	print 'Need data sourcename,  and debug'
        sys.exit(1)
...
try:
	...
	#endIF
	#providerID = AdminConfig.getid(proScopeValue +"/DataSource:" + jdbcProviderName + "/")
	providerID = AdminConfig.getid("/DataSource:" + jdbcProviderName + "/")
#endTry
except:
	if debug == "YES":
...
if (len(providerID) == 0):
	# provider does not exist
        if debug == "YES":
           	print "DataSource = ", jdbcProviderName, " was not found."
        sys.exit(0)
#endIf

# otherwise we have a provideer and need to delete it
if debug == "YES":
	print "Datasource ready for delete."
#endIF
...
try:
	...
	AdminTask.deleteDatasource(providerID)
#endTry
except:
	print "Exception with AdminTask.deleteDataSource call."
	sys.exit(1)
#endExcept
