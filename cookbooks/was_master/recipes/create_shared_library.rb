...
execute "Create Shared Library" do
  command "#{node['was']['install_home']}bin/wsadmin.sh -conntype SOAP -host #{ node['was']['host'] } -port #{ node['was']['soap_port'] } -lang jython -user #{node['was']['was_user']} -password '#{node['was']['was_pass']}' -f #{node['was']['jython_path']}/#{jython_script_name} \"#{node['was']['library_scope']}\" \"#{node['was']['node_name']}\" \"#{node['was']['server_or_cluster_name']}\" \"#{node['was']['shared_library_name']}\" \"#{node['was']['shared_library_classpath']}\" \"#{node['was']['shared_library_description']}\" \"#{node['was']['native_library_path']}\" \"#{node['was']['isolated_class_loader']}\" \"YES\""
end
