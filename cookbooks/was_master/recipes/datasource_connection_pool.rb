...
execute "Datasource Set Min Max Connection Pool" do
  command "#{node['was']['install_home']}bin/wsadmin.sh -conntype SOAP -host #{ node['was']['host'] } -port #{ node['was']['soap_port'] } -lang jython -user #{node['was']['was_user']} -password '#{node['was']['was_pass']}' -javaoption \"#{node['was']['java_option_first']}\" -javaoption \"#{node['was']['java_option_second']}\" -f #{node['was']['jython_path']}/#{jython_script_name} \"#{node['was']['data_source_name']}\" \"#{node['was']['data_source_min']}\" \"#{node['was']['data_source_max']}\" \"#{node['was']['connection_timeout']}\" \"#{node['was']['unused_timeout']}\" \"#{node['was']['aged_timeout']}\" \"#{node['was']['reap_time']}\" \"#{node['was']['purge_policy']}\" \"#{node['was']['scope_level']}\" \"#{node['was']['data_source_cluster_or_server']}\" \"#{node['was']['node_name']}\" \"#{node['was']['provider_name']}\" \"YES\""
end

#1Data source name,  2min connections, 3maximum connections, connection timeout ,  unused timeout, aged timeout, reap time,Purge Policy ( FailingConnectionOnly, EntirePool), scope ( Cell, Cluster, Node, Server ), server or cluster name ( not used for node scope – use none ), node name ( not used for Cell or Cluster scope ), jdbc provider name associated with datasource, debug
