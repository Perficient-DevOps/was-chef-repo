...
execute "Datasource Set Min Max Connection Pool" do
  command "#{node['was']['install_home']}bin/wsadmin.sh -conntype SOAP -host #{ node['was']['host'] } -port #{ node['was']['soap_port'] } -lang jython -javaoption #{node['was']['jvm_size']} -user #{node['was']['was_user']} -password '#{node['was']['was_pass']}' -f #{node['was']['jython_path']}/#{jython_script_name} \"#{node['was']['data_source_name']}\" \"#{node['was']['data_source_min']}\" \"#{node['was']['data_source_max']}\" \"#{node['was']['reap_time']}\" \"#{node['was']['unused_timeout']}\" \"#{node['was']['aged_timeout']}\" \"#{node['was']['purge_policy']}\" \"#{node['was']['scope_level']}\" \"#{node['was']['data_source_cluster_or_server']}\" \"#{node['was']['node_name']}\" \"#{node['was']['provider_name']}\" \"YES\""
end
