...
execute "Set JDBC Provider" do
  command "#{node['was']['install_home']}bin/wsadmin.sh -conntype SOAP -host #{ node['was']['host'] } -port #{ node['was']['soap_port'] } -lang jython -user #{node['was']['was_user']} -password '#{node['was']['was_pass']}' -f #{node['was']['jython_path']}/#{jython_script_name} \"#{node['was']['cluster_or_server_name']}\" \"#{node['was']['db_type']}\" \"#{node['was']['jdbc_provider']}\" \"#{node['was']['data_source_implementation_type']}\" \"#{node['was']['provider_name']}\" \"#{node['was']['provider_jar_path']}\"  \"#{node['was']['provider_description']}\" \"#{node['was']['implementation_class_name']}\" \"#{node['was']['scope_level']}\" \"#{node['was']['node_name']}\" \"YES\""
end
