...
wsadmin 'Create JDBC Provider' do
  ...
  not_if { ::File.readlines( "#{node['was']['install_home']}profiles/#{node['was']['dmgr_profile_name']}/config/cells/#{node['was']['cell_name']}/resources.xml").grep("#{node['was']['provider_name']}").empty? }
end
